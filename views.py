from .models import Tarefa, Grupos, Sub_Grupos, User
...
class MostraSubGrupo(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        id_subgrupo = subgrupo_especifico.id
        ...
        context = {'join_subgrupo': join_subgrupo, 'pk': pk, 'search_input': busca, 'id_grupo': id_grupo, 'id_subgrupo': id_subgrupo}

        return render(request, "login/mostra_subgrupo.html", context)


def pega_get(request, grupo):
    item = Grupos.objects.get(id=grupo)
    id_grupo = item.id
    grupo_obj = get_object_or_404(Grupos, id=grupo)
    id_usuario = get_object_or_404(User, id=request.user.id)
    form = Sub_GruposForm(request.POST or None)

    if form.is_valid():
        form = form.save(commit=False)
        form.grupo = grupo_obj
        form.user = id_usuario
        form.save()
        return redirect(f'/subgrupos/{id_grupo}')

    context = {'item': item, 'grupo': grupo, 'id_grupo': id_grupo, 'form': form}
...
def pega_get_tarefa(request, grupo_sub, subgrupo_tar_id):
    item = Sub_Grupos.objects.get(id=subgrupo_tar_id)
    join_subgrupo = item.tarefa_subgrupos.all()
    id_subgrupo = item.id
    grupo_obj = get_object_or_404(Grupos, id=grupo_sub)
    subgrupo_obj = get_object_or_404(Sub_Grupos, id=subgrupo_tar_id)
    id_usuario = get_object_or_404(User, id=request.user.id)
    ...
    if form.is_valid():
        form = form.save(commit=False)
        form.grupo = grupo_obj
        form.subgrupo_tar_id = subgrupo_obj
        form.user = id_usuario
        form.save()
        return redirect(f'/mostra_subgrupo/{id_subgrupo}')

    context = {'join_subgrupo': join_subgrupo, 'item': item, 'id_subgrupo': id_subgrupo, 'form': form}
